...
class TestShouldStopAdaptive:
    ...
    def test_stops_when_cv_below_target(self):
        ...
        assert (
            should_stop_adaptive([1000, 1001, 999], rep=2, min_reps=3, target_cv=0.5)
            is True
        )
    ...
    def test_continues_when_cv_above_target(self):
        ...
        assert (
            should_stop_adaptive([1000, 1050, 950], rep=2, min_reps=3, target_cv=0.5)
            is False
        )
    ...
    def test_never_stops_before_min_reps(self):
        ...
        assert (
            should_stop_adaptive([1000, 1000], rep=1, min_reps=3, target_cv=0.5)
            is False
        )

    def test_disabled_when_target_zero(self):
        assert (
            should_stop_adaptive([1000, 1001, 999], rep=2, min_reps=3, target_cv=0.0)
            is False
        )
    ...
    def test_stops_at_min_reps_boundary(self):
        ...
        assert (
            should_stop_adaptive([1000, 1001, 1000], rep=2, min_reps=3, target_cv=0.5)
            is True
        )
    ...
    def test_allows_stop_after_min_reps(self):
        ...
        assert (
            should_stop_adaptive(
                [1000, 1001, 999, 1000, 1001], rep=4, min_reps=3, target_cv=0.5
            )
            is True
        )
