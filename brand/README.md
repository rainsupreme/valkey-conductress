...
`conductress --version` prints the 24px sunset lockup above the version line
when standard output is an interactive terminal, and only the version line when
it is a pipe, when `NO_COLOR` is set, or when `TERM` is `dumb`. The copy it
prints lives in the package as `src/conductress/assets/lockup-24.ansi` and is
generated by this script with `--palette sunset --size 24 --no-labels`;
regenerate it whenever the mark changes so the banner and this kit agree.
